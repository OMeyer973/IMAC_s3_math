...
int main(int argc, char **argv)
{
  ...
  while(loop){
    ...
    // Q 4-6 SCALING THE IMAGE
    // moving the image so it's center is in 0 0 (top-left corner)
    Eigen::Matrix3d Hcentering = Eigen::Matrix3d::Identity();
    Hcentering(0,2) = -frame.size().width*0.5;
    Hcentering(1,2) = -frame.size().height*0.5;

    // scaling image to appear smaller in the screen
    float scaleFactor = 0.7;
    Eigen::Matrix3d zoom = Eigen::Matrix3d::Identity();
    zoom(0,0) = zoom(1,1) = scaleFactor;

    // moving the small image back to the center of the screen
    Eigen::Matrix3d HcenteringInv = Eigen::Matrix3d(Hcentering);
    HcenteringInv(0,2) *= -1;
    HcenteringInv(1,2) *= -1;


    // Q 7 STABILIZATING THE IMAGE
    // on cherche à stabiliser une rotation + translation (pas de scale)
    // matrice de transformaton de la forme
    ...
    // on cherche la pseudo inverse pour résoudre avec les moindres carrés
    //  x =  A^+ * b
    //  x = (A^T * A)^(-1) * A^T b
    ...
    //solve the problem (moindres carres)
